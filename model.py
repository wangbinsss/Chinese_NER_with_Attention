...
class BiLSTM_CRF_CNN_Attention(object):
    def __init__(self, args, embeddings, pos_embeddings, tag2label, pos2id, vocab, paths, config):
        ...
        self.embeddings_pos = pos_embeddings
        self.CRF = args.CRF
    ...
    def lookup_layer_op(self):
        ...
        with tf.variable_scope("pos_embedding"):
            _pos_embeddings = tf.Variable(self.embeddings_pos,
                                          dtype=tf.float32,
                                          trainable=self.update_embedding,
                                          name="_pos_embeddings")
            pos_embeddings = tf.nn.embedding_lookup(params=_pos_embeddings,
                                                    ids=self.pos_ids,
                                                    name="pos_embeddings")
        self.pos_embeddings = tf.nn.dropout(pos_embeddings, self.dropout_pl)
    ...
    def biLSTM_layer_op(self):
        with tf.variable_scope("bi-lstm"):
            ...
            self.output = tf.nn.dropout(self.output, self.dropout_pl)
    ...
    def predict_one_batch(self, sess, poss, seqs):
        ...
        :return: label_list
                 seq_len_list
        """
        feed_dict, seq_len_list = self.get_feed_dict(seqs, poss, dropout=1.0)
        if self.CRF:
    ...
    def evaluate(self, label_list, seq_len_list, data, epoch=None):
        ...
        model_predict = []
        for label_, (sent, tag, pos) in zip(label_list, data):
